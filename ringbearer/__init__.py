...
Hold the ring and speak. The capture reaches this bridge one of two ways --
the app's "Webhook only" gesture posts it straight here, or its MCP sandbox
agent calls this bridge's send_to_assistant tool -- and the bridge posts it
into your Telegram DM with the assistant, as you, from your own Telegram
account, so one thread carries the whole conversation and the assistant
replies exactly as it would to a typed message.
...
Pieces, if you ever need one alone:
  ...
  python ringbearer.py probe    client-side diagnostic: talk to every configured
                            route the way the phone does (dry run; --live sends;
...
Endpoints:
  ...
  <MCP_MOUNT>/webhook   Receiver for the app's "Webhook only" gesture: audio or
                        transcript in, same bearer token.
